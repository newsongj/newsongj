...
import { fetchRetreatInfo, fetchGyoguList, fetchPatientRoomMembers, submitPatientRoom } from '@api/retreat';
...
const ReasonInput = styled('input')(({ theme }) => ({
    ...
    verticalAlign: 'middle',
    '&::placeholder': { color: theme.custom.colors.text.medium },
    '&:focus': { border: `2px solid ${theme.custom.colors.primary._500}` },
...
const PatientRoomPage: React.FC = () => {
    ...
    const [allMembers,  setAllMembers]  = useState<PatientRoomMember[]>([]);
    ...
    const [drafts,      setDrafts]      = useState<Map<number, PatientRoomDraft>>(new Map());
    ...
    const loadMembers = useCallback(async (g?: number, t?: number) => {
        setLoading(true);
        try {
            const data = await fetchPatientRoomMembers(g !== undefined ? { gyogu: g, team: t } : undefined);
            setAllMembers(data);
        } catch (err: any) {
    ...
    const updateDraft = useCallback((memberId: number, patch: Partial<PatientRoomDraft>) => {
        setDrafts((prev) => {
    ...
    const handleSubmit = useCallback(async (memberId: number, isUpdate: boolean) => {
        ...
        try {
            await submitPatientRoom(memberId, {
    ...
    }, [getDraft, loadMembers, allMembers, gyogu, teamFilter]);
        ...
        <PageWrapper>
            ...
            {snackbar.open && (
                <Alert
                    ...
                    sx={{ position: 'fixed', bottom: 24, left: '50%', transform: 'translateX(-50%)', zIndex: 9999, minWidth: 240 }}
                >
                    {snackbar.message}
                </Alert>
            )}
